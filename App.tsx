...
const App: React.FC = () => {
  // 檢查 API KEY 是否真的存在 (Vite 會在編譯時替換 process.env.API_KEY)
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <p className={`text-[10px] font-bold ${isKeyConfigured ? 'text-slate-400' : 'text-orange-500'}`}>
                  {isKeyConfigured ? 'Flash 模型已就緒' : 'API 金鑰未設定 (請檢查 Vercel)'}
                </p>
              </div>
            </div>
          </div>
          <div className="hidden md:flex gap-2">
            <span className="px-3 py-1 bg-slate-100 rounded-lg text-xs font-bold text-slate-500 border border-slate-200 flex items-center gap-1">
              <ShieldCheck className={`w-3 h-3 ${isKeyConfigured ? 'text-green-500' : 'text-slate-300'}`} /> 
              系統狀態: {isKeyConfigured ? '正常' : '待設定'}
            </span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto p-4 md:p-6">
        ...
        <div className="bg-white rounded-3xl p-6 shadow-xl border border-slate-200 mb-8 overflow-hidden relative">
          ...
          <div className="flex items-center gap-2 mb-6 text-slate-700">
            ...
            <h2 className="text-lg font-black">查詢設定</h2>
          </div>
        ...
        {!hasSearched ? (
          ...
          <div className="bg-white border-2 border-orange-100 p-10 rounded-[40px] text-center shadow-2xl">
            {error === "API_KEY_MISSING" ? (
              <>
                <Key className="w-16 h-16 text-orange-500 mx-auto mb-6" />
                <h3 className="text-2xl font-black text-slate-800 mb-2">尚未設定 API 金鑰</h3>
                <p className="text-slate-500 font-bold mb-6">請在 Vercel 的 Environment Variables 中新增 API_KEY。</p>
                <div className="bg-slate-50 p-4 rounded-xl text-left text-xs font-mono border border-slate-200 mb-6 max-w-md mx-auto">
                  1. 前往 Vercel 專案設定<br/>
                  2. 找到 Environment Variables<br/>
                  3. Key: API_KEY<br/>
                  4. Value: (貼上您的 Gemini Key)<br/>
                  5. 重新 Redeploy 專案
                </div>
              </>
          ...
          <div className="space-y-6">
            ...
            <div className="flex justify-between items-end px-2">
              ...
              <button onClick={() => {
                ...
                alert('已複製到剪貼簿！');
              }} className="text-xs font-black text-slate-500 border-2 border-slate-100 px-4 py-2 rounded-xl flex items-center gap-2 bg-white hover:bg-slate-50 transition-all shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-4">
              {stocks.length > 0 ? stocks.map((stock, idx) => (
                ...
                <div className="py-24 text-center bg-white rounded-[40px] border border-slate-100 shadow-sm">
                  ...
                  <p className="text-slate-300 text-sm font-bold mt-2">AI 正努力在海量數據中搜尋，請嘗試調整日期</p>
                </div>
              )}
            </div>
